...
The simplest entry point is `runProgram` from `@mettascript/core`. Give it a source string; get back one result group per `!`-query.
...
When you want an object you can feed over time, use the `MeTTa` class from `@mettascript/hyperon`. Its space is live: atoms you add are visible to later queries.
