...
A naive `match` over a space is a linear scan, which does not scale. MeTTaScript has five tools for large knowledge bases, in increasing specialization.
...
The memo store uses structural token keys and a token trie, not recursive pretty-printed strings. Runtime rules are keyed by a whole-world rule version, so a cached answer is not reused after a runtime equation changes. A branching recursive function whose only mutable dependency is `match` can also be tabled. Its key includes a whole-world atom-space content version, and every write to `&self` or a named space moves later calls to a new key. `get-atoms`, state operations, imports, writers, and other impure expressions remain excluded. Registered sync and async grounded operations are treated as effectful unless they are the unchanged implementation of a known-pure built-in.
...
If a non-ground tabled call directly re-enters the same active variant, MeTTaScript switches that call to local-linear completion. It replays the active table's known answers, re-runs the pure producer until no new canonical answers are added, and returns the fixed-point answer set once. Non-cyclic calls still preserve ordered bags and duplicate answers.

Static analysis chooses between moded tabling and compiled depth-first search. Independent overlapping calls such as `fib(n-1)` and `fib(n-2)` stay table-first. If a later recursive call consumes a clause-local field introduced by an earlier goal, its table keys fan out with that answer; these dependent joins run on the compiler first. Bounded backward chaining has that shape because its second proof search consumes the size and type produced by the first. The compiled path keeps the same clause-major result order and occurs check.

When every successful clause shares a result shell such as `MkSized`, the compiler can omit fields that are projections of the call inputs. It can also defer one unbound output field when static dataflow proves that the field does not affect matching, guards, or recursive calls. Generated code then constructs that output only after child searches succeed. Runtime admission rejects bound or aliased outputs. If either proof fails, the same default evaluator uses ordinary compiled search, the bounded table space, or the interpreter.
...
This is not Picat-style mode-directed tabling. Picat can keep only `min`, `max`, or other selected answers for a mode declaration. MeTTaScript does not infer that semantic choice automatically. There is one default evaluator. Admission and fallback happen inside it, with no tabling mode needed for normal execution or benchmarks.

## The flat interned KB

For very large, mostly-ground knowledge bases, `FlatKB` (from `@mettascript/core`) stores atoms as a contiguous array of `Int32` tokens with symbols and grounds interned to ids (modeled on MORK's representation). Equality becomes an integer compare and traversal is a cache-friendly linear scan. It matches a pattern with one-sided flat unification:
...
When you have a _large_ KB and a _non-selective_ query whose _result set is small_ (a needle in a haystack), `ParallelFlatMatcher` (from `@mettascript/node`) puts the flat KB's tokens in a `SharedArrayBuffer` and scans them across a pool of `worker_threads`, claiming work via an `Atomics` counter:

```ts
import { FlatKB, sym, expr, gint, variable, type Atom } from "@mettascript/core";
