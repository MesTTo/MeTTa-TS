...
A completed-table hit replays answers, not the producer's allocation history. Ground answers consume no fresh-variable IDs. A non-ground answer is standardized apart by allocating one ID for each variable that remains live in that answer. Recharging the complete producer counter would reconstruct the original overlap recurrence inside the bookkeeping. For relational Fibonacci, that gives exponential counter growth despite linear table evaluation. Answer-local replay keeps both evaluation work and counter growth linear in the number of call variants.
