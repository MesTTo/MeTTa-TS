...
Interpreted evaluation under a cursor follows the same law. A `metta` or `metta-thread` call whose expected type is `%Undefined%` pulls one full-evaluator answer per request through an owned continuation, and interpreted rule alternatives reduce and publish one at a time behind a one-alternative lookahead, so a single-answer chain keeps its direct return path and `once` closes the unvisited tail. A typed expected result stays on the batch path because its check reads the world at completion time. A producer fault after one held alternative delivers that alternative before the fault, exactly as a direct grounded stream does.
